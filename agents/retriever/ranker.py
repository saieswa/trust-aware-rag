...
2. Base FAISS semantic cosine similarity score.
3. Cross-query agreement bonus.
4. Section & Page relevance boost (boosting Page 1 / Abstract for title & author queries).
5. Penalizing bibliographies and appendix noise.
"""
...
def rank_results(state: RetrieverState) -> Dict[str, Any]:
    ...
    logger.info(
        f"[RETRIEVAL] Reranked {len(ranked_results)} candidate chunks for query: {query!r} (doc_id={target_doc_id!r}). "
...
def select_top_evidence(state: RetrieverState) -> Dict[str, Any]:
    ...
    logger.info(f"[RETRIEVAL] Selected top {len(top_evidence)} evidence chunks for Critic Agent.")
